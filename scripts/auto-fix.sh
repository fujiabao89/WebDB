...
#
# 依赖: gh CLI (已登录), claude CLI (已登录), jq
#
# 工作流程:
...
# 检查 jq 是否可用
command -v jq >/dev/null 2>&1 || { echo "❌ jq 未安装，请先安装: https://stedolan.github.io/jq/"; exit 1; }
...
run_claude_fix() {
    ...
    local claude_output
    claude_output=$(echo "$review_content" | claude -p "你是 WebDB 项目的自动修复 Agent。阅读以下 Codex 审查意见，仅修复 P1(HIGH) 和 P2(MEDIUM) 问题。禁止: 改架构/API/Schema、动密钥凭证、改 CI/CD 配置、无关重构。仅编辑源代码文件，不要执行 git commit 或 git push——脚本会自动完成提交和推送。" --dangerously-skip-permissions --max-turns 15 --output-format text 2>&1) || true

    echo "$claude_output"

    # 检查是否有变更
    if [ -n "$(git status --porcelain -- . ':(exclude).auto-fix' 2>/dev/null)" ]; then
        echo ""
        echo "📦 检测到代码变更，提交并推送..."
        git add .
...
> 🤖 此评论由 auto-fix.sh 自动生成"
    ...
    local review_id
    review_id=$(echo "$review_json" | jq -r '.id' 2>/dev/null || echo "0")
    local review_time
    review_time=$(echo "$review_json" | gh api --input - --jq '.submitted_at' 2>/dev/null || echo "")
