...
- 运营：fork PR 或 secret 缺失时 job 空转但有显式 notice；仅在 `HAS_SECRET == 'true'` 时调用外部 DeepSeek API（有少量成本与延迟）；`git fetch`/`git checkout` 依赖仓库公开可读（private 仓库需另行授予读取凭据，本 ADR 不覆盖）。
