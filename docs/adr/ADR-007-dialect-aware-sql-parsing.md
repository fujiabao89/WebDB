...
> 状态：已接受｜日期：2026-07-11｜修订：2026-08-13｜批准人：`fujiabao89`、WEB-44 Owner
...
MySQL session mode 的生产门禁采用“同一 session 读取并验证”方案：SQL policy 只使用当前 lexer/Omni 共同支持的模式（`ANSI_QUOTES=false`、`NO_BACKSLASH_ESCAPES=false`）；Adapter 每次从连接池取得专用 `*sql.Conn` 后，必须在只读事务和任何用户 SQL 前，从该连接读取 `@@SESSION.sql_mode`，将其规范化为词法模式并与 policy 模式比较。读取失败、格式异常、未知 mode、会改变解析语义但当前 parser 不支持的 mode，或比较不一致时，必须拒绝请求并销毁该连接。检查、只读事务和目标查询必须使用同一连接，连接复用后必须重新检查；不得依赖 DSN 声明、全局变量或另一条 session，也不主动改写目标数据库的 `sql_mode`。

## 后果

解析、分类和拒绝案例必须有两种引擎的回归与模糊测试。PostgreSQL 回归测试必须覆盖 `TABLE t` 按等价 `Select` AST 放行，以及 `TABLE`/`SELECT` 携带锁定子句、修改型 CTE、多语句、未知节点或解析失败时拒绝。

MySQL 回归与模糊测试必须分别验证 lexer 和 AST parser，并验证两者的调用顺序。lexer 测试至少覆盖可执行注释正例、字符串/标识符/普通注释/hint/行注释反例、反斜杠及引号模式、控制字符、未知版本、未闭合输入和极端长度；任何 lexer 错误、可执行注释、AST 失败或 session mode 验证失败都不得访问目标数据库。真实 MySQL 集成测试必须覆盖池连接复用后的 `NO_BACKSLASH_ESCAPES` 与 `ANSI_QUOTES` 拒绝路径。固定 Omni 精确版本、跨平台构建和完整许可证清单须在正式依赖批准前由隔离 Spike 证明。DML/DDL 留给独立策略路径。
...
| 2026-08-13 | WEB-44 Owner | MySQL 在每次执行前从同一池化 session 读取并验证 `@@SESSION.sql_mode`；未知、不支持或不一致时在用户 SQL 前 fail-closed 并销毁连接。 |
