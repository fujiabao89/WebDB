...
3. **执行方式无自动提交路径**：弃用 `anomalyco/opencode/github` action 的 `github run` 子命令（其含 `branchIsDirty → git add && git commit && git push`）。改为直接调用固定版本的 `opencode run --no-share`，该子命令无自动 commit/push 逻辑。**已实测验证的 CLI 参数行为（2026-08-08，PR #41 真实 runner）**：① v1.18.15 的 `-f/--file` 为 array 选项，会贪婪吞并其后所有位置参数，消息文本必须置于 `-f` 之前（`opencode run <消息> -f <文件>`；`-f <文件> <消息>` 会把消息误当文件路径报 `File not found`）；② 非 TTY 下 `run` 会 `await stdin.text()`，GitHub Actions 步骤 stdin 若不显式重定向会导致进程挂起（曾实测 30 分钟无输出），必须加 `< /dev/null`；③ 建议加 `timeout 600` 兜底并在失败时输出 model-error.log 尾部（GitHub 日志对进行中 run 不可下载）。
...
- 验证：聚合入口 `test-opencode-review.sh`（非安全自检）依次运行 `test-opencode-workflow.sh`（20 项对抗样例）、`test-review-output.sh`（98 例：含 required-conclusion、严格 finding 字段/路径/行号校验、验收矩阵/结论结构、结论语义如 P0-P2+APPROVE 拒/矩阵失败或证据不足+APPROVE 拒/RC 需阻断依据、结构块恰好 1+1 与顺序、details 结构状态机扫描（属性/大小写/嵌套/多余 closing/同行/额外 summary 全拒）、完整文档布局（块外仅徽章/置信度/结论三行，块外 finding/标题/表格/说明全拒）、徽章整行 fullmatch（P4/P10/前缀/后缀/括号后尾文全拒）、置信度完整行（唯一、倒数第二非空行、级别高/中/低、风险非空））、`test-fetch-ci-rollup.sh`（16 例：退出码×JSON schema 矩阵、正向分步断言与参数解析）；`validate-opencode-workflow.sh` 通过；YAML/UTF-8 校验；`python -m py_compile`；`git diff --check`；actionlint/shellcheck（如环境可用）。**真实 runner 端到端验证（2026-08-08，PR #41 head `d15d43e`，run `31252929990`）**：固定 CLI 安装（v1.18.15 + sha256 校验）成功；模型步骤仅持 `DEEPSEEK_API_KEY`（无 `GITHUB_TOKEN`）；DeepSeek 调用成功且模型正确输出 ESCALATE（required 程序约束生效）；`validate-review-output.py` 校验通过；评论经受信发布步骤成功发布且无 session 链接；全步骤 success 且无任何 commit/push 副作用。期间修复的两个运行期问题：`external_directory` 权限规则按后定义优先匹配，配置中的 `"*": "deny"` 兜底会吞掉全部放行规则（曾致模型读不到 `$RUNNER_TEMP/review-context`），移除兜底（未匹配落到内置 `ask`→非交互自动拒绝，仍 fail closed）并加入 review-context 精确放行模式；`opencode run` 需 `< /dev/null` + `timeout` 兜底（见决策 3）。
