...
> **状态：已接受（Owner 接受：2026-08-13，fujiabao89）｜实施已完成（WEB-35/36/37/38/39 均已合并）**
>
> Owner 已对 D01–D18 全部决策逐项给出明确结论（D05a/D08/D11/D13/D14/D15f 为专项决定，其余按推荐选项），**记录于本文件 §17**。**本契约已被 Owner 接受（2026-08-13，fujiabao89）**。WEB-34 本任务仅做契约设计与批准，不注册路由、不改运行时代码、不写测试；实施与契约测试由并发任务 WEB-35/36/37/38/39 承接（§19 矩阵），现已全部合并完成：连接列表与 Schema 浏览（WEB-36，`internal/browse`）、只读执行/续页与审计确认（WEB-35，`internal/executionhttp`）、VerifiedSortPlan 与 Service-owned 分页（WEB-38，`internal/queryplan`/`internal/pagination`，ADR-014/015 迁移完成）、高保真工作台（WEB-37）与 Compose E2E 验收（WEB-39）。续页路由已随 ADR-014/015 迁移完成后注册。本文档不自行关闭 WEB-34，Linear 状态由 Owner 更新。
>
...
>
> **阅读前须知**：文中"不得""必须"等措辞为**已接受契约**的约束（Owner 已接受 2026-08-13）。文中标注的"尚未实现/目标态"以本文件撰写时点（2026-08-08）为准；ADR-014/015 迁移与路由注册已由 WEB-35/36/37/38/39 完成。
...
- **独立可核验审批证据**：Linear [WEB-34](https://linear.app/webdb/issue/WEB-34/p0-06a最小-http-契约高保真范围映射与-owner-gate) 任务及其审批评论（决策者 fujiabao89、时间 2026-08-08、D01-D18 对应关系）。Owner 于 2026-08-13 确认接受，契约状态已更新为已接受；续页路由仍须 ADR-014/015 迁移完成后方可注册。
- **本契约状态：已接受（2026-08-13，fujiabao89）**；未注册路由、未修改运行时代码、未编写测试；是并发实施任务（WEB-35/36/37/38/39）的共同设计基线，实施与 §19 契约测试由这些任务承接。
...
Owner（fujiabao89）已于 **2026-08-08** 对 D01–D18 逐项给出决策，记录于下表（"Owner 决定"列）。**注意：下表为 2026-08-08 的决策记录；契约已于 2026-08-13 由 Owner 正式接受（见文首状态与 §1.1）**——契约已接受，但尚未实施、未注册路由、未通过测试验收；实施与测试由 WEB-35/36/37/38/39 承接。每个决策含：决策问题、推荐选项、备选项、权衡、下游影响。
...
| P0-06 验收项 | 本提案覆盖 | 状态 |
|---|---|---|
| UI 只经 API 显示已授权连接/Schema，绝不暴露连接密钥 | §6/§7 连接安全 DTO + 授权条件；`host/port/secret_ref/secret_version` 默认不公开 | 提案已覆盖（D02/D03 决策已记录） |
| 编辑器运行只读 SQL；策略拒绝、超时、取消和 429 可理解呈现 | §8/§10/§12 错误码、499、429+Retry-After、本地取消状态 | 提案已覆盖（D15 系决策已记录） |
| 结果使用服务端分页，单页默认最多 500 行，不全量加载 | §9 续页契约 + §13 页上限默认 100/最大 500；续页路由 ADR-014/015 前置 | 提案已覆盖（D06a/D11 决策已记录）；**注册前置未达，WEB-38 待实现** |
| P0 主流程可从 Compose 演示库完成：连接→浏览→查询→分页→审计 | §5.2 可信 Principal + §6-§11 全链路 | 决策已记录（D01b）+ 待后续实现验证（WEB-35/36/37/38/39） |
| 高保真页面不能反向要求 API 暴露敏感字段 | §3 浏览器不得接收清单 + D17 | 提案已覆盖（D17 决策已记录） |
| 登录/协作/DML/DDL/导出/行编辑/移动端/浏览器直连仍排除 | §3 非目标 | 提案已覆盖 |
| 取消、超时、审计失败语义冻结 | §10/§11/§12 | 提案已覆盖（D13/D14 决策已记录） |
