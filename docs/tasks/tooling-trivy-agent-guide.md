...
> 状态：In Progress｜Linear Task：`WEB-42`｜风险：High｜分支：`chore/WEB-42-trivy-baseline`｜独立审查：未参与实现的安全 Reviewer
...
- Draft PR 创建后必须完成一次真实 GitHub Actions 全量运行；在此之前 CI 验收项保持“证据不足”，不得宣称完成。
