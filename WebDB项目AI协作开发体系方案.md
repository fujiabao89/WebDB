...
每条问题必须按 P0-P3 给出文件:行号、可验证事实、影响、复现/推理和最小修复建议。低风险纯假设且证据不足时列为“待确认”而不阻断；权限、SQL 安全边界、数据隔离、密钥、生产数据或不可恢复操作等高风险关键事实无法安全确认时，必须升级人工。
最后输出验收矩阵（通过/失败/证据不足）、漏测建议、文本结论（APPROVE / REQUEST CHANGES / ESCALATE）和置信度；APPROVE 只是审查文本结论，不代表在 GitHub 等平台执行批准或合并。
```
...
stateDiagram-v2
  ...
  REVIEW --> NEEDS_HUMAN: P0/P1、意见冲突或高风险关键事实证据不足
...
| 幻觉式结论 | 要求事实-推断分栏，并由独立 Agent/确定性工具抽查 | 低风险纯假设的证据不足转待确认且不阻断；高风险关键事实无法确认时升级人工 |
